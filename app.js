...
try {
    ...
    console.error("Error al inicializar Firebase. AsegTienes razón en que la función `Reiniciar Juego Total` no estaba reseteando los coloresúrate de usar un servidor web.", error);
...
const participantesRef = database de los participantes, y hay problemas con el estilo del panel de rol personal.

**Corregiré:**

1.  **Reiniciar Colores:** Modificaré `resetButton` en `app.js` para borrar la.ref('participantes'); 
...
const mensajePrincipal = document.getElementById('mensaje-principal asignación de color (`color: null`) de todos los participantes.
2.  **Visibilidad y Estilo del Panel Personal:** Aseguraré que el panel (`personal-role-panel`) **solo** se muestre si el usuario'); 
...
const adminLoginButton = document.getElementById tiene un color asignado y que los estilos neón se apliquen correctamente.

Aquí están las versiones actualizadas de `app.js` y `style.css`.

## 1. `app.js` (Reset de colores y visibilidad del panel)

He modificado el listener de `participantesRef.child(ANONYMOUS_USER_ID)` y la función `resetButton`.

```javascript
// app.js

// ... (config('admin-login-button');
...
const ejectedCrewmate = document.getElementById('ejected-crewuración y referencias sin cambios) ...
...
try {
    ...
    console.error("Error al inicializarmate-icon');
...
function updateVoteDisplay(jugadoresSnapshot, votosDetalleSnapshot) {
    ...
    for (const color of coloresJugadores) {
        const votosActuales = jugadores[color] Firebase. Asegúrate de usar un servidor web.", error);
    alert("Error al conectar a la base de datos. Por favor, asegúrate de abrir la página desde un servidor web.");
    throw new Error("Firebase no inicializado.");
}
...
const ANONYMOUS_USER_ID = get ? jugadores[color].votos || 0 : 0;
        ...
        } else if (color !== 'skip' && !(jugadores[color] && jugadores[color].eliminado) && votosActuales === maxVotos && maxAnonymousUserId();
document.getElementById('user-id-display').textContent = `Tu ID: ${ANONYMOUS_USER_ID}`; 
 ...
 * Función que actualiza la visualización de votos, barras e iconos.
 */
function updateVoteDisplay(jugadoresVotos > 0) {
        ...
        if (contadorElement) {
             ...
             votantes.forEach(votanteId => {
                 ...
                 Snapshot, votosDetalleSnapshot) {
    const jugadores = jugadoresSnapshot.val();
    const votosDetalle = votosDetalleSnapshot.val() || {};
    const participantesData = participantesCache; 

    let maxconst colorVotante = participante && coloresTripulantes.includes(participante.color) ? participante.color : 'skip';
...
// ----------------------------------------------------
// Listener Combinado
let currentJugadoresSnapshotVotos = -1;
    let jugadorMasVotado = null;
    let totalVotos = 0;
...
// =========================================================
// LÓGICA DE TEMPORIZADOR        if (jugadores[color] && jugadores[color].eliminado === true && botonElement) {
            botonElement.classList.add('eliminado');
        } else if (botonElement) {
             botonElement.classList.remove('eliminado');
        }
...
function obtenerJugadorMasVotado(jugadoresData) {
    ...
    if (esEmpate) {
        jugadorMasVotado = 'EMPATE';
        isEliminado = false;
    } else if (jugadorMasVotado !== ' = votosActuales;
            jugadorMasVotado = color;
        ...
        if (contadorElement) {
             contadorElement.innerHTML = '';
             const votantes = Object.keys(votosDetalle).filter(id => votosDetalle[id].voto === colorNADIE') {
        isEliminado = true; 
    }
...
function showExpulsionResult(ejectedColor, ejectedRole, ejectedName) {
    ...
    if (ejectedColor === 'SKIP' || ejectedColor === 'EMPATE') {
        ...
        expulsionPopup.classList.add('skip-ejected');
             ...
             votantes.forEach(votanteId => {
                 const participante = participantesData[votanteId];
                 // El color del votante es su color asignado o 'skip' si no tiene uno        ejectedCrewmate.style.display = 'none'; // Ocultar el crewmate
    } else {
    ...
    // 6. Mostrar el resultado (Líder Actual el popup después de 5 segundos (debe coincidir con la duración de la animación CSS)
    setTimeout(() => {
...
function finalizarVotacion() {
    ...
    temporizadorElement.textContent = "00:0)
    let liderTexto = jugadorMasVotado === "EMPATE" 
        ? "EMPATE" 
        : jugadorMasVotado ? jugadorMasVotado.toUpperCase() : "NADIE";0 - Votación Cerrada";
...
jugadoresRef.on('value', (snapshot) => {
    currentJugadoresSnapshot = snapshot;
    if (currentVotosDetalle (eliminado y mensaje)
...
function actualizarTemporizador(tiempoFin) {
    ...
    timerInterval = setInterval(() => {
        ...
        if (tiempoRestante <= 0) {
            finalizarVotacion();
            return;Snapshot) updateVoteDisplay(currentJugadoresSnapshot, currentVotosDetalleSnapshot);
});
...
function votar(personaje) {
    participantesRef.child(ANONYMOUS_USER_ID).once('value').then(participanteSnap => {
        ...
        // --- 1. NUEVA RESTRICCIÓN: Jugador eliminado// Asegurarse de que el mensaje principal se actualice solo después del pop-up
         estadoRef.once('value').then(snap => {
            mensajePrincipal.textContent = snap.val().mensaje;
         });
...
function finalizarVotacion() {
    clearInterval(timerInterval);
    configRef.update({ votoActivo: false });
    temporizadorElement.textContent = no puede votar ---
        if (miColor && coloresTripulantes.includes(miColor)) {
            ...
            // Si el jugador no tiene color asignado (es observador "00:00 - Votación Cerrada";
...
// Controla la visibilidad de los botones de Admin
function updateAdminButtonsVisibility(), puede votar
            performVoteChecks(personaje);
        }
    });
}
...
function performVoteChecks(personaje) {
    ...
    configRef.child('votoActivo').once('value').then(snap => {
        ...
        // Si vota por alguien que ya está eliminado (excluyendo 'config) {
    if (isAdmin) {
        participantPanel.style.display = 'flex';
        adminLoginButton.style.display = 'none';
...
// Función que muestra la notificación de rol a pantalla completa
function showRoleNotification(rol) {
    roleNotification.textContent = `¡TUskip')
        if (personaje !== 'skip') {
...
configRef.on('value', (snapshot) => {
    const config = snapshot ROL ES: ${rol.toUpperCase()}!`;
    roleNotification.classList.remove('crewmate', 'impostor');
    roleNotification.classList.add(rol === 'impostor' ? 'impostor' : 'crewmate');
    roleNotification.style.display = 'flex';
...
function votar(personaje) {
    participantesRef.child(ANONYMOUS_USER_ID).once('value').then(participanteSnap => {
        ...
        if (miColor && coloresTripulantes.includes(miColor)) {
            ...
            // Si el jugador no tiene color asignado (.val();
    ...
    if (config.tiempoFin > Date.now() && config.votoActivo) { 
        actualizarTemporizador(config.es observador), puede votar
            performVoteChecks(personaje);
        }
    });
}
...
function checkAndRestrictAccess(participantesData) {
    ...
    const tieneColor = participantesData[ANONYMOUS_USER_ID] && coloresTripulantes.includes(participantes
        const votoRef = (personaje === 'skip') 
            ? jugadoresRef.child('skip/votos') 
            : jugadoresRef.child(`${personaje}/votos`);
        ...
        // Si vota por alguien que ya está eliminado (exData[ANONYMOUS_USER_ID].color);
...
function setupParticipantcluyendo 'skip')
        if (personaje !== 'skip') {
            jugadoresRef.child(personaje).once('value').then(jugadorSnap => {
                if (jugadorSnap.val() && jugadorSnap.val().eliminado) {
                    alert(`¡${personaje.toUpperCase()} ya ha sido eliminado! No puedes votar por él.`);
...
participantesRef.child(ANONYMOUS_USER_ID).on('value', (snapshot) => {
    ...
    if (participante && participante.rol) {
        ...
        const miColor = participante.color;
        ...
        if (miColor && coloresTripulantes.includes(miColor)) {
            ...
            if (participante.rol === 'impostor') {
                myRoleDisplay.classList.add('im config = snapshot.val();
...
postor');
            ...
            myCrewmateIcon.classList.add(miColor);

        } else {
             // Ocultar si no tiene color asignado (no es un jugador principal)
...
function checkAndRestrictAccess(participantesData) {
    ...
    if (jugadoresConColor >= 5 && !tieneColor && !isAdmin) {
        ...
        accessRestrictionMessage.style.display = 'none';
        document.getElementById('app-container').style.display = 'checkAndRestrictAccess(participantesData); 
    ...
    if (participantesArray.length ===flex';
        return false;
    }
}
            ...
            <div class="admin-actions">
                ...
                <div class="color-assignment">
                    ${coloresTripulantes.map(color => `
                        <button class="color-btn ${color}" data-id="${p.id}" data-color="${color}" ${p.colorrol: 'sin asignar',
                color: null
            });
        }
    });
...
participantesRef.child(ANONYMOUS_USER_ID).on('value', (snapshot) => {
    ...
    if (participante && participante.rol) {
        ...
        // --- LÓGICA DE PANEL PERSONAL (Visibilidad con color) ---
        myCrewmateIcon.classList.remove(...coloresTripulantes);
        myCrewmateIcon.classList.remove === color ? 'disabled' : ''}>${color.charAt(0).toUpperCase()}</button>
                    `).join('')}
    ...
    document.querySelectorAll('skip');
        ...
        if (participante.color && coloresTripulantes.includes(participante.color)) {
            ...
            if (participante.rol === 'impostor') {
             ...
             // Ocultar si no tiene color asignado
             personalRolePanel.style.display = 'none';
        }
    } else {
         personalRolePanel.style.display = 'none';
...
function updateParticipantDisplay(participantesData) {
    ...
    // 2. RENDERIZADO DEL ADMIN
    participantListContainer.innerHTML = ''; ('.role-btn').forEach(button => {
...
function asignarColor(userId, color) {
    ...
    if (participantesArray.length === 0) {
        participantListContainer.innerHTML = '<p class="admin-message">No hay participantes conectados actualmente.</p>';).update({ color: color });
        });
    } else {
         // Quitar el color
...
adminLoginButton.addEventListener('click', () => {
    ...
    if (password === 'zxz') { // La clave secreta
        ...
        participantesRef.once('                    <button class="color-btn skip" data-id="${p.id}" data-color="null" ${p.color === undefined || p.color === null ? 'disabled' : ''}>X</button>
                </div>
                <button class="role-btn tripulante" data-id="${p.id}" data-rol="tripulante">Tripulante</button>
                <button class="role-btn impostor" data-id="${p.id}" data-rol="impostor">Impostor</button>
            </div>
...
continueButton.addEventListener('click', () => {
    if (!isAdmin) { alert// 4. Función de asignación de color (para el ADMIN)
function asignarColor(userId, color) {
    if (!isAdmin) return;
...
// 3. Reiniciar JUEGO TOTAL (Solo Admin - FIX: RESETEANDO COLORES)
resetButton.addEventListener('click', () => {
    ...
    jugadoresRef.set(jugadoresReset).then(() => {
        ...
        // 2. Resetear el rol y EL COLOR de TODOS los participantes
        participantesRef.once('value').then(snapshot => {
            const updatesRoles = {};
            snapshot.forEach(childSnapshot => {
                updatesRoles[`${childSnapshot.key}/rol`] = 'sin asignar';
                updatesRoles[`${childSnapshot.key}/color`] = null; // <<< FIX: Resetea color
            });
            participantesRef.update(updatesRoles);
        });
         ...
         alert("Juego reiniciado. Todos los jugadores están de vuelta, roles y colores fueron borrados.");
    });
});
