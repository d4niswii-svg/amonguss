...
// const clearChatButton = document.getElementById('clear-chat-button'); // <<-- ELIMINADO

// ** NUEVAS REFERENCIAS: TAREAS **
const taskPanel = document.getElementById('task-panel');
const taskListContainer = document.getElementById('task-list-container');
const taskCompletionStatus = document.getElementById('task-completion-status'); // NUEVA REFERENCIA
...
// const toggleTaskControlButton = document.getElementById('toggle-task-control-button'); // <<-- ELIMINADO
...
function verificarFinDePartida(origen = 'expulsion') {
    ...
    // 2. Lógica de Victoria (Impostores VS Tripulantes)
    ...
    // 3. Lógica de Victoria (Tareas) - Solo se comprueba si el origen no es de asesinato/expulsión.
    if (!juegoTerminado && origen === 'tarea' && tareasRef && taskCompletionStatus) {
        const tareasCompletadas = taskCompletionStatus.dataset.completed === 'true';
        if (tareasCompletadas) {
            mensajeVictoria = "¡TAREAS COMPLETADAS! La tripulación ha ganado.";
            ganador = 'crewmate';
            juegoTerminado = true;
        }
    }
        ...
        if (resultado.esEliminado) {
            ...
            jugadoresRef.child(`${ejectedColor}/eliminado`).set(true).then(() => {
                 ...
                 estadoRef.update({ 
                    ...
                    verificarFinDePartida('expulsion');
                 });
            });
...
function updateAdminButtonsVisibility(config) {
    ...
    if (isAdmin) {
        ...
        // if (clearChatButton) clearChatButton.style.display = 'block'; // Limpiar Chat <<-- ELIMINADO
        if (clearAllParticipantsButton) clearAllParticipantsButton.style.display = 'block'; // NUEVO BOTÓN
        
        // Control de Tareas <<-- ELIMINADO
        // if (toggleTaskControlButton) toggleTaskControlButton.style.display = 'none'; 
        ...
        const imp2Btn = document.getElementById('set-impostor-2');
        const impAutoBtn = document.getElementById('set-impostor-auto');
        
        if(imp1Btn) { 
            imp1Btn.style.display = 'block'; 
            imp1Btn.classList.toggle('active-impostor-count', currentImpostors === 1); 
        }
        if(imp2Btn) { 
            imp2Btn.style.display = 'block'; 
            imp2Btn.classList.toggle('active-impostor-count', currentImpostors === 2); 
        }
        if(impAutoBtn) { 
            impAutoBtn.style.display = 'block'; 
            impAutoBtn.classList.toggle('active-impostor-count', currentImpostors === 'auto'); 
        }
         ...
         // if (clearChatButton) clearChatButton.style.display = 'none'; // <<-- ELIMINADO
         if (clearAllParticipantsButton) clearAllParticipantsButton.style.display = 'none';
         // if (toggleTaskControlButton) toggleTaskControlButton.style.display = 'none'; // <<-- ELIMINADO
         ...
         if (document.getElementById('set-impostor-2')) document.getElementById('set-impostor-2').style.display = 'none';
...
function setupParticipantTracking() {
    ...
    // *** MODIFICACIÓN CLAVE: ELIMINAR onDisconnect PARA LOGRAR LA PERSISTENCIA TOTAL
    // userRef.onDisconnect().update({ conectado: false }); // <--- ELIMINADO
    ...
    userRef.once('value').then(snapshot => {
        ...
        // El estado 'conectado' se establece en TRUE a menos que el admin lo haya marcado como 'expulsado'
        const isExpulsado = existingData && existingData.rol === 'expulsado';
        
        const updates = { 
            conectado: isExpulsado ? false : true, 
...
function updateParticipantDisplay(participantesData) {
    ...
    participantesArray.forEach(p => {
        ...
        let jugadorEliminado = p.rol === 'expulsado' || false;
        // Solo comprueba el estado de eliminado de la votación
        ...
        const statusText = jugadorEliminado ? ' (ELIMINADO)' : (p.conectado ? ' (ONLINE)' : ' (OFFLINE)');
        const roleAndColorText = `${p.rol ? p.rol.toUpperCase() : 'SIN ASIGNAR'} (${p.color || 'N/A'})`;


        pElement.innerHTML = `
            <span class="user-index-name ${p.conectado ? 'online' : 'offline'} ${jugadorEliminado ? 'ejected-player' : ''}">${index}. <strong>${nombreMostrado}</strong> ${statusText}</span>
...
function asignarColor(userId, color) {
    ...
    if (color) {
        participantesRef.once('value').then(snapshot => {
            ...
            // *** IMPORTANTE: El rol se reinicia para forzar la reasignación de roles después de un cambio de color. ***
...
function adminKillPlayer(userId, color, name) {
    ...
    jugadoresRef.child(`${color}/eliminado`).set(true).then(() => {
         // *** MODIFICACIÓN CLAVE: Establece conectado a false para que se muestre como OFFLINE/Eliminado
        ...
        participantesRef.once('value').then(snapshot => {
            ...
            participantesRef.update(updates).then(() => {
                ...
                configRef.update({ 
                    ...
                    votoSecreto: false, 
                    numImpostors: 1,
                    taskControl: true, // Casillas ABIERTAS por defecto
...
// ELIMINADO: Toggle de Control de Tareas


// ELIMINADO: Botón para Limpiar Chat (ADMIN)
