...
// ** NUEVA REFERENCIA ADMIN **
const assignImpostorButton = document.getElementById('assign-impostor-button');
...
jugadoresRef.on('value', (snapshot) => {
    ...
    for (const color of coloresJugadores) {
        ...
        // 2. Aplicar estilo de eliminado (y el nuevo ojo muerto)
    ...
    if (totalVotos === 0) {
        ...
        // También limpiar el color de resultado anterior
        resultadoFinalElement.classList.remove('impostor-found', 'crewmate-ejected');
    } else {
...
// =========================================================
// ** FUNCIÓN NUEVA: Verificar Condición de Victoria **
// =========================================================
function verificarFinDePartida() {
    // Necesitamos leer tanto el nodo jugadores como el de participantes si queremos la sincronización de roles, 
    // pero para la victoria, solo necesitamos 'jugadores' (colores con esImpostor: true)
    jugadoresRef.once('value').then(snapshot => {
        ...
        for (const color of coloresJugadores) {
            const jugador = jugadores[color];
            // Solo contamos jugadores NO ELIMINADOS y no 'skip'
            if (color !== 'skip' && jugador && !jugador.eliminado) {
        ...
        if (impostoresRestantes === 0) {
            mensajeVictoria = "¡VICTORIA DE LOS TRIPULANTES! Todos los Impostores han sido expulsados.";
            juegoTerminado = true;
...
// =========================================================
// ** FUNCIÓN MODIFICADA: Finalizar Votación **
// =========================================================
function finalizarVotacion() {
    // 1. Desactivar votoActivo
    ...
    jugadoresRef.once('value').then(snapshot => {
        ...
        // Limpiar el color de resultado anterior
        resultadoFinalElement.classList.remove('impostor-found', 'crewmate-ejected');

        if (resultado.esEliminado) {
            jugadoresRef.child(`${resultado.nombre}`).once('value').then(jugadorSnap => {
                const esImpostor = jugadorSnap.val().esImpostor || false;

                // 1. Marcar como eliminado
                jugadoresRef.child(`${resultado.nombre}/eliminado`).set(true);

                // 2. Determinar el mensaje final y el estilo CSS
                const mensajeFinal = esImpostor
                    ? `¡${resultado.nombre.toUpperCase()} fue el Impostor! ENCONTRADO.`
                    : `¡${resultado.nombre.toUpperCase()} fue expulsado! No era un Impostor.`;
                
                // Aplicar la clase de efecto visual al resultado FINAL (solo al final de la votación)
                resultadoFinalElement.classList.add(esImpostor ? 'impostor-found' : 'crewmate-ejected');

                // 3. Actualizar estado y verificar victoria
                estadoRef.update({
                    ultimoEliminado: resultado.nombre,
                    mensaje: mensajeFinal
                }).then(() => {
                    // Verificar fin de partida DESPUÉS de la expulsión
...
function updateAdminButtonsVisibility(config) {
    if (!isAdmin) {
        ...
        assignImpostorButton.style.display = 'none'; // NUEVO BOTÓN
        return;
    }
    ...
    if (isVoting) {
        startTimerButton.style.display = 'none';
        continueButton.style.display = 'none';
        allowMultipleVoteButton.style.display = 'inline-block'; 
        assignImpostorButton.style.display = 'none';
    } else if (isFinished) {
        startTimerButton.style.display = 'none';
        continueButton.style.display = 'inline-block';
        allowMultipleVoteButton.style.display = 'none';
        assignImpostorButton.style.display = 'inline-block';
...
configRef.on('value', (snapshot) => {
    ...
    if (config.tiempoFin > Date.now() && config.votoActivo) {
        ...
        document.body.classList.add('emergency-meeting'); // Efecto visual
        ...
        if (personaje !== 'skip') {
            jugadoresRef.child(personaje).once('value').then(jugadorSnap => {
                if (jugadorSnap.val().eliminado) {
                    alert(`¡${personaje.toUpperCase()} ya ha ha sido eliminado! No puedes votar por él.`);
                    return;
                }
                performVote();
            });
...
resetButton.addEventListener('click', () => {
    ...
    for (const color of coloresJugadores) {
        if (color === 'skip') {
            ...
            // ** RESETEAMOS también el flag esImpostor **
            jugadoresReset[color] = { votos: 0, eliminado: false, esImpostor: false }; 
        }
    }

    // 1. Resetear jugadores (votos, eliminado, esImpostor)
    jugadoresRef.set(jugadoresReset).then(() => {
        ...
        estadoRef.update({ ultimoEliminado: null, mensaje: "¡Juego Reiniciado! ¡Asigna un Impostor!" });
...
// =========================================================
// ** NUEVA FUNCIÓN: ASIGNAR IMPOSTOR POR COLOR (Admin) **
// =========================================================
assignImpostorButton.addEventListener('click', () => {
    if (!isAdmin) { alert('Requiere privilegios de administrador.'); return; }
    
    // 1. Preguntar por el color
    const colorImpostor = prompt("Introduce el color (ej: rojo) del nuevo Impostor:");
    
    // Validación básica de color
    if (!coloresJugadores.includes(colorImpostor) || colorImpostor === 'skip') {
        alert("Color no válido. Introduce un color de jugador (amarillo, azul, blanco, rojo, verde).");
        return;
    }

    // 2. Resetear todos los 'esImpostor' a false y asignar el nuevo
    jugadoresRef.once('value').then(snapshot => {
        const updates = {};
        const jugadores = snapshot.val();
        
        // Desmarcar al impostor anterior
        for (const color of coloresJugadores) {
            if (color !== 'skip' && jugadores[color] && jugadores[color].esImpostor) {
                updates[`${color}/esImpostor`] = false;
            }
        }
        
        // Asignar el nuevo Impostor
        updates[`${colorImpostor}/esImpostor`] = true;

        jugadoresRef.update(updates).then(() => {
             alert(`¡El Impostor ha sido asignado al color ${colorImpostor.toUpperCase()}! (¡Se mantiene oculto!)`);
             // ** IMPORTANTE: ESTA ASIGNACIÓN SOLO OCURRE EN EL NODO JUGADORES, NO EN PARTICIPANTES **
             // El admin debe correlacionar quién tiene el color para asignar el rol en el panel de participantes.
             estadoRef.update({ mensaje: `El Impostor ha sido asignado. ¡A votar!` });
        });
    });
});
